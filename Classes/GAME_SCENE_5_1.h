#ifndef __GAME_SCENE_5_1_H__
#define __GAME_SCENE_5_1_H__

#include "cocos2d.h"
USING_NS_CC;

class GameScene5_1 : public cocos2d::Layer
{
public:
	static cocos2d::Scene* createScene();

	virtual bool init();
	CREATE_FUNC(GameScene5_1);

	//메뉴 화면으로 돌아가기
	void GameScene5_1::menuCallback(cocos2d::Ref* sender);
	...
	//이벤트 발생 받기
	bool GameScene5_1::knowEvent(int mapLevel, int eventCode);
...
private:
	...
	//이벤트 발생 (cleanBox 실행후 Box가 된다)
	bool cleanBox = false, window = false, drainCleaner = false, toilet = false;
	bool pot = false, spigot = false, womanFrame = false, door = false;
	bool bath = false, manFrame = false;

	//이벤트 진행
	bool cleanBoxItem = false, boxItem = false, cleanWindow = false, findBox = false;;
	bool drainCleanerItem = false, toiletItem = false, potItems = false, spigotItem = false;
	
	//컷인창 켜짐
	bool windowCutInOn = false;

	//톱니바퀴, 다음맵 이동
	bool clockGear5 = false, NextMap = false;

	//창문 컷인
	Sprite* windowCutIn;
	Sprite* womanFrameCutin;
